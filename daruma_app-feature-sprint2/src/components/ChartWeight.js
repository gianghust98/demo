...
const Y_SCALE = 2.7
const HEIGHT = (CHART_HEIGHT * Y_SCALE) + (MARGIN_VERTICAL * 2) // svgのheight

export default class ChartWeight extends Component {
  ...
  getPathData(data, dateLength) {
    ...
    for (let i = 0; i < dateLength; i += 1) {
      ...
      if (oneDayData.length > 0) {
        const oneDayDataValue = oneDayData.map(elm => elm.bodyWeight)
  ...
  convertYValue = y => Math.round(((CHART_HEIGHT - y) * Y_SCALE) + MARGIN_VERTICAL)
