...
- (OIDTokenRequest *)tokenExchangeRequest: (NSString *) tokenEndpointAuthMethod {
    return [self tokenExchangeRequestWithAdditionalParameters:nil tokenEndpointAuthMethod:tokenEndpointAuthMethod];
}

- (OIDTokenRequest *)tokenExchangeRequestWithAdditionalParameters:(NSDictionary<NSString *, NSString *> *)additionalParameters
  ...
  if (!_authorizationCode) {
    ...
    if([tokenEndpointAuthMethod isEqualToString:@"client_secret_post"]){
      return [[OIDTokenRequestPost alloc] initWithConfiguration:_request.configuration
